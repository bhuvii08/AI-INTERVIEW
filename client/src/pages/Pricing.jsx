...
function Pricing() {
  ...
  const handlePayment = async (plan) => {
    try {
      ...
      if (!razorpayKey) {
        alert("Payment is not configured. Missing VITE_RAZORPAY_KEY_ID in client/.env")
        setLoadingPlan(null)
        return
      }
      ...
      if (!amount) {
        alert("Please choose a paid plan to continue.")
        setLoadingPlan(null)
        return
      }
      ...
      if (!isRazorpayLoaded || !window.Razorpay) {
        alert("Unable to load Razorpay checkout. Please check internet and try again.")
        setLoadingPlan(null)
        return
      }
      ...
      if (!result?.data?.id) {
        alert("Failed to initialize payment order. Please try again.")
        setLoadingPlan(null)
        return
      }
     ...
     if (status === 401) {
      ...
      alert(error?.response?.data?.message || "Payment failed to start. Please try again.")
     }
     console.log(error)
     setLoadingPlan(null);
    }
