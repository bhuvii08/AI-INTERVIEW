...
function Step1SetUp({ onStart }) {
    ...
    const handleUploadResume = async () => {
        ...
        try {
            ...
            console.log(error)
            setResumeError(error?.response?.data?.message || "Resume analysis failed. Please try again.")
            setAnalyzing(false);
        }
    }
    ...
    const handleStart = async () => {
        ...
        const safeExperience = experience.trim();

        if (!userData) {
            setStartError("Please login first to start interview.");
            return;
        }

        if (!safeRole || !safeExperience) {
            setStartError("Role and experience are required.");
            return;
        }

        if (!navigator.onLine) {
            setStartError("No internet connection. Please reconnect and try again.");
            return;
        }
        ...
        setLoading(true)
        try {
           const result = await axios.post(ServerUrl + "/api/interview/generate-questions" , {role: safeRole, experience: safeExperience, mode , resumeText, projects, skills } , {withCredentials:true}) 
           console.log(result.data)
           if(userData){
            dispatch(setUserData({...userData , credits:result.data.creditsLeft}))
           }
           setLoading(false)
           onStart({ ...result.data, assistantVoice })
            ...
            if (status === 401) {
                setStartError("Session expired. Please login again.");
            } else if (status === 400 && serverMessage) {
                setStartError(serverMessage);
            } else if (!error?.response) {
                setStartError("Could not reach server. Please ensure backend is running on port 8000.");
            } else {
                setStartError(serverMessage || error?.message || "Failed to start interview. Please try again.");
            }
            setLoading(false)
        }
    }
                ...
                <motion.div
                    ...
                    <div className='space-y-6'>
                        ...
                        <div className='space-y-2'>
                            <p className='text-sm text-gray-600'>Choose AI assistant voice</p>
                            <div className='grid grid-cols-2 gap-3'>
                                <button
                                    type='button'
                                    onClick={() => setAssistantVoice("female")}
                                ...
                                <button
                                    type='button'
                                    onClick={() => setAssistantVoice("male")}
                                    className={`py-2 rounded-xl border text-sm font-medium transition ${
                                        assistantVoice === "male"
                                            ? "bg-green-600 text-white border-green-600"
                                            : "bg-white text-gray-700 border-gray-200 hover:border-green-300"
